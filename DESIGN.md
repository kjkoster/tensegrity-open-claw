...
- **Rust, no_std, on `esp-hal`** for the ESP32-C3 (Seeed XIAO).
- **3.3V logic throughout (outbound):** the ESP32-C3 drives the IRLz44N gates and
  ULN2003 inputs directly — no level shifters on the MCU outputs.
- **Logic-level caveat (inbound):** the ESP32-C3 is strictly 3.3V. The RS-485
...
| `nrf24` | `embedded-nrf24l01` | ✅ | verify build for `riscv32imc-unknown-none-elf` |
| `led-lamp-rf` | — | 🔨 | bespoke; port packet format from MiLight C/C++ refs |
| `fixture-output` | — (opt. `smart-leds`, `stepper`) | 🔨 | `smart-leds` only if onboard LED is addressable; stepper table trivial |
...
- [ ] Confirm all chosen crates build for `riscv32imc-unknown-none-elf` against the pinned `esp-hal`
...
Hardware BOM
- [ ] ESP32-C3 (Seeed XIAO)
- [ ] Confirm onboard LED type (addressable WS2812 vs plain GPIO/PWM)

Firmware
- [ ] no_std Rust project on `esp-hal`
...
- [ ] Control onboard LED via DMX channels as test output (`smart-leds` if addressable, else LEDC)
- [ ] Stub channel handlers for later builds

Validation
- [ ] Control onboard LED colour + brightness via QLC+ over local WiFi
...
- [ ] Wire nRF24L01 to ESP32-C3 via SPI
