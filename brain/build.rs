...
fn main() {
    ...
    let out = Path::new(&env::var("OUT_DIR").unwrap()).join("scenes.rs");
    fs::write(&out, render(&scenes)).unwrap();
}
...
fn parse_patch(engine: roxmltree::Node) -> BTreeMap<u32, Patched> {
        ...
        .filter(|node| node.is_element() && node.tag_name().name() == "Fixture")
    {
        ...
        if patch
            .insert(id, Patched { name, address, channels })
            .is_some()
        {
...
fn render(scenes: &[(String, Vec<(u32, u8)>)]) -> String {
